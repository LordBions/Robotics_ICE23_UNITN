 ...
 * @brief script used to move the ur5
 * @version 0.1
 * @date 2023-06-12
 * 
...
/**
 * @brief topic where 'moviment_module' is subscribed
 * 
 */
#define sub_task_commander "/planner/taskCommander"

/**
 * @brief topic that 'moviment_module' publishes envetResult messages
 * 
 */
#define pub_task_resulter "/motion/taskResulter"

/**
 * @brief publisher which talks to locosim joint
 * 
 */
#define pub_joint_commander "/ur5/joint_group_pos_controller/command"

/**
 * @brief It is a topic where a client could command real robot gripper [Only in real robot, no simulation]
 * 
 */
...
/**
 * @defgroup movement_commands
 * @brief constants used to identify commands defined in movement module
 * @{
 */
...
// security commands
/**
 * @defgroup movement_security_commands
 * @brief constants used to provide security in the communication
 * @{
 */
#define command_handshake 9 ///< command to begin the handshake sequence
#define command_auth_key 10 ///< command to request the authorization code from the movement module
/**@}*/
...
// 
#define max_joint_speed 1.5     ///< max joint speed
#define min_joint_speed -1.5    ///< min joint speed
#define max_diameter_ext 130 ///< max fingers space  
#define min_diameter_ext 22 ///< min  fingers space

bool verbose_flag = false;       ///< flag used to enable deep logging
...
/**
 * @brief movement subscriber
 * 
 */
ros::Subscriber sub_task_commander_handle;

/**
 * @brief ServiceClient object
 * @note Another way to communicate instead of topics
 */
ros::ServiceClient client_gripper_commander_handle;

/**
 * @brief Communication message that planner send to movement
 * 
 */
motion::legoTask task_command;

/**
 * @brief Ack that movement uses to respond
 * 
 */
motion::eventResult evento;

/**
 * @brief Used to store the success of the task
 * 
 */
int risultato_var;

/**
 * @brief Structure used to coordinate messages between planner and movement
 * @param command_id            used to understand which task movement must execute
 * @param start_moment          initial time
 * @param interval_moment       duration of the task
 * @param busy                  shows if the movement is performing a task or not      
 */
...
/**
 * @brief Structure to coordinate messages between planner and movement
 * 
 */
...
void AuthKeySend(); //< executes the second part of handshake and sends the authorization key to planner

bool verifyAuthKey(int auth_key); ///< routine that checks if planner have sent the correct authorization code

int generateNextRandom(); ///< generates the next authorization code 
